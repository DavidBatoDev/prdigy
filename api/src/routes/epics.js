...
/**
 * GET /api/epics
 * List all epics for a roadmap (with query param)
 */
router.get("/epics", verifySupabaseJwt, readLimiter, async (req, res, next) => {
  try {
    const { roadmap_id } = req.query;

    if (!roadmap_id) {
      return res.status(400).json({
        error: { message: "roadmap_id query parameter is required" },
      });
    }
 ...
 * List all epics for a roadmap (legacy route for backward compatibility)
 */
router.get(
  "/roadmaps/:roadmapId/epics",
  verifySupabaseJwt,
  readLimiter,
  async (req, res, next) => {
    try {
 ...
 * Create a new epic (legacy route for backward compatibility)
 */
router.post(
  "/roadmaps/:roadmapId/epics",
  verifySupabaseJwt,
...
/**
 * PATCH /api/epics/reorder
 * Bulk reorder epics within a roadmap
 */
router.patch(
  "/epics/reorder",
  verifySupabaseJwt,
  ...
  async (req, res, next) => {
    try {
      const { roadmap_id, reorders } = req.body;
      ...
      for (const reorder of reorders) {
        if (!reorder.epic_id || typeof reorder.new_order_index !== "number") {
          return res.status(400).json({
            error: {
              message: "Each reorder must have epic_id and new_order_index",
            },
          });
        }
      }

      // Update each epic's position
      for (const reorder of reorders) {
        await supabaseAdmin
          .from("roadmap_epics")
          .update({ position: reorder.new_order_index })
          .eq("id", reorder.epic_id)
          .eq("roadmap_id", roadmap_id);
      }

      res.json({ data: { message: "Epics reordered successfully" } });
    } catch (error) {
      next(error);
    }
  },
 ...
 * Update epic position (legacy route for backward compatibility)
 */
router.patch(
  "/epics/:id/reorder",
  verifySupabaseJwt,
