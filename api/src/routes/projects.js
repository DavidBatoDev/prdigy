 ...
 */
router.post('/', verifySupabaseJwt, requirePersona('client'), async (req, res, next) => {
  try {
...
router.patch('/:id', verifySupabaseJwt, async (req, res, next) => {
  try {
    ...
    const isOwner = project.client_id === req.user.id || project.consultant_id === req.user.id;
    if (!isOwner) {
 ...
 */
router.post('/:id/assign-consultant', verifySupabaseJwt, requirePersona('admin'), async (req, res, next) => {
  try {
    ...
    if (!consultant?.is_consultant_verified) {
      return res.status(400).json({ error: { message: 'User is not a verified consultant' } });
    }
