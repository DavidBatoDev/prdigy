 ...
 */
router.post('/', verifySupabaseJwt, requirePersona('consultant', 'admin'), async (req, res, next) => {
  try {
    const { project_id, milestone_id, amount, payer_id, payee_id, description } = req.body;

    if (!project_id || !amount || !payer_id || !payee_id) {
      return res.status(400).json({ 
        error: { message: 'Missing required fields: project_id, amount, payer_id, payee_id' } 
      });
    }
 ...
 */
router.patch('/:id/complete', verifySupabaseJwt, requirePersona('consultant', 'admin'), async (req, res, next) => {
  try {
    const { data, error } = await supabaseAdmin
      .from('payment_checkpoints')
      .update({ 
