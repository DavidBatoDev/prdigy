 ...
 */
router.get("/features/:featureId/tasks", verifySupabaseJwt, readLimiter, async (req, res, next) => {
  try {
 ...
 */
router.get("/tasks/:id", verifySupabaseJwt, readLimiter, async (req, res, next) => {
  try {
    const { id } = req.params;
 ...
 * Create a new task
 */
router.post("/features/:featureId/tasks", verifySupabaseJwt, writeLimiter, async (req, res, next) => {
  try {
    const { featureId } = req.params;
    const {
      title,
    ...
    if (checklist) {
      ...
      for (const item of checklist) {
        ...
        if (typeof item.completed !== "boolean") {
          return res.status(400).json({
            error: { message: "Each checklist item must have a completed boolean field" },
          });
        }
      }
    }
 ...
 * Update a task
 */
router.patch("/tasks/:id", verifySupabaseJwt, writeLimiter, async (req, res, next) => {
  try {
    const { id } = req.params;
    const {
      title,
 ...
 */
router.patch("/tasks/:id/reorder", verifySupabaseJwt, writeLimiter, async (req, res, next) => {
  try {
    const { id } = req.params;
 ...
 */
router.patch("/tasks/:id/assign", verifySupabaseJwt, writeLimiter, async (req, res, next) => {
  try {
    const { id } = req.params;
 ...
 */
router.patch("/tasks/:id/unassign", verifySupabaseJwt, writeLimiter, async (req, res, next) => {
  try {
    const { id } = req.params;
 ...
 * Delete a task
 */
router.delete("/tasks/:id", verifySupabaseJwt, writeLimiter, async (req, res, next) => {
  try {
    const { id } = req.params;
