...
interface RoadmapCanvasHeaderProps {
  ...
  setOpenEpicTabs: Dispatch<SetStateAction<string[]>>;
  onCloseEpicTab: (epicId: string) => void;
  onShare?: () => void;
  onExport?: () => void;
}
...
export function RoadmapCanvasHeader({
  ...
  return (
    <div className="bg-gray-100 border-b border-gray-200 flex items-center justify-between w-full shrink-0 z-10 overflow-hidden">
      <div className="flex items-center pl-6 overflow-x-auto flex-1 no-scrollbar h-full">
        <button
          onClick={() => {
            setViewMode("roadmap");
        ...
        {openEpicTabs.length > 0 && <div className="h-8 w-px bg-gray-300 shrink-0" />}
